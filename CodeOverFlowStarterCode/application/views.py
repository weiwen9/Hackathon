...
def modifying_expenses(expense_id):
    ...
    expense = Expenses.query.filter(
        Expenses.user == current_user.email, Expenses.expense_id == expense_id
    ).first()
    if not expense:
        ...
        if request.method == "GET":
            form = EditExpense(
                data={
                    ...
                    "date": date(
                        int(expense.date_purchase[:4]),
                        int(expense.date_purchase[5:7]),
...
def deleting_expenses():
    ...
    expense = Expenses.query.filter(Expenses.user == current_user.email,
        Expenses.expense_id == request.form.get("id")
    ).first()
