...
class SignUp(FlaskForm):
    ...
    password = PasswordField(
        "Password*", validators=[DataRequired(), Length(min=8, max=24)]
    )
    ...
    def validate_password(self, password):
        if not re.fullmatch(
            "^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]+$", password.data
        ):
            raise ValidationError(
                "Password needs to contain at least one letter, number, and special character."
            )
...
class EditExpense(FlaskForm):
    id = HiddenField("")
    type = SelectField(
        "Type of expense", choices=[(typ, typ) for typ in Type_of_expense]
    )
    description = StringField(
        "Description of the expense.", validators=[DataRequired("Description required")]
    )
    date = DateField("Purchase Date", format="%Y-%m-%d", validators=[DataRequired()])
    amount = FloatField(
        "Amount (S$)",
        validators=[
            DataRequired("Invalid amount, input should be a number greater than 0.")
        ],
    )
    ...
    def validate_amount(self, amount):
        if amount.data <= 0:
            raise ValidationError(
                "Invalid amount, input should be a number greater than 0."
            )
